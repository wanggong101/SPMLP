...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: dict):
        ...
        self.evaluate_on_gpu = cfg["TEST"].get("USE_GPU",
                                               True)  # evaluate on gpu or cpu (gpu is faster but may cause OOM)
    ...
    def build_train_dataset(self, cfg: dict):
        ...
        data_file_path = "{0}/data_in{1}_out{2}.pkl".format(cfg["TRAIN"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"],
                                                            cfg["DATASET_OUTPUT_LEN"])
        index_file_path = "{0}/index_in{1}_out{2}.pkl".format(cfg["TRAIN"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"],
                                                              cfg["DATASET_OUTPUT_LEN"])
    ...
    def build_val_dataset(cfg: dict):
        ...
        """
        data_file_path = "{0}/data_in{1}_out{2}.pkl".format(cfg["VAL"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"],
                                                            cfg["DATASET_OUTPUT_LEN"])
        index_file_path = "{0}/index_in{1}_out{2}.pkl".format(cfg["VAL"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"],
                                                              cfg["DATASET_OUTPUT_LEN"])
    ...
    def build_test_dataset(cfg: dict):
        ...
        data_file_path = "{0}/data_in{1}_out{2}.pkl".format(cfg["TEST"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"],
                                                            cfg["DATASET_OUTPUT_LEN"])
        index_file_path = "{0}/index_in{1}_out{2}.pkl".format(cfg["TEST"]["DATA"]["DIR"], cfg["DATASET_INPUT_LEN"],
                                                              cfg["DATASET_OUTPUT_LEN"])
    ...
    # def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
    #     iter_num = (epoch - 1) * self.iter_per_epoch + iter_index
    #     prediction, real_value, smooth_loss = self.forward(data=data, epoch=epoch, iter_num=iter_num, train=True)
    #
    #     # 反归一化
    ...
    #
    #     # 主损失
    #     main_loss = self.metric_forward(self.loss, [prediction_rescaled, real_value_rescaled])
